...
written by `scripts/derive_core_tools.py` into `blended/agent/
core_tools.py` — a generated module, because the addon ships the
package without `_evaluate/` and must know the set at runtime — and a
pure test asserts that constant equals the derivation from the log, so
the offered set cannot drift from the evidence.
...
CORE_MODULE_PATH = Path(__file__).with_name("core_tools.py")
...
def core_ops() -> tuple[str, ...]:
    """The pinned core set, from the generated module."""
    from blended.agent.core_tools import CORE_OPS

    return CORE_OPS


def write_core_module(core: tuple[str, ...], counts: dict[str, int], path: Path = CORE_MODULE_PATH) -> None:
    """Write the generated module. Never edit it by hand."""
    lines = [
        ...
        f"MINIMUM_BRIEFS_USING_OP = {MINIMUM_BRIEFS_USING_OP} distinct gate-passing briefs in",
        "_evaluate/iterations.jsonl. tests/pure/test_tool_disclosure.py asserts",
        'this equals the derivation from the log."""',
        "",
        "CORE_OPS = (",
    ]
    lines.extend(f'    "{name}",  # {counts[name]} brief(s)' for name in core)
    lines.append(")")
    path.write_text("\n".join(lines) + "\n")
