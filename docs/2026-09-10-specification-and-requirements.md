...
| OPS-4 | Every created object MUST be linked into the scene before use, or the depsgraph has no instance and measurements silently return stored values. | `src/blended/ops/primitives.py:56` | `tests/blender/test_transform_ops.py` |
...
| AGT-5 | A turn that changes the scene MUST declare a plan first: with `require_plan` on, `run_python` without a plan MUST be refused with the refusal text (`PLAN_REQUIRED_TOOLS`). | `src/blended/agent/plan.py:43` | `tests/pure/test_turn_plan.py` |
...
| AGT-7 | Plan progress MUST be reported as a clamped 0.0–1.0 fraction plus `step n/total`, and MUST survive an out-of-range step index without raising. | `src/blended/agent/plan.py:62` | `tests/pure/test_turn_plan.py` |
